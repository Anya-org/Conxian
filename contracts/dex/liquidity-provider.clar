...
(define-map liquidity-positions { pool: principal, provider: principal } {
  liquidity-amount: uint, pool-shares: uint, last-deposit: uint, rewards-earned: uint, rewards-claimed: uint, fee-tier: uint
})

(define-map provider-stats { provider: principal } {
  total-liquidity: uint, total-rewards: uint, pool-count: uint, first-provision: uint
})

(define-map reward-history { provider: principal } {
  timestamp: uint, pool: principal, amount: uint
})
    ...
    (let ((existing-position (default-to
      ...
      (let ((stats (default-to { total-liquidity: u0, total-rewards: u0, pool-count: u0, first-provision: burn-block-height } (map-get? provider-stats { provider: tx-sender }))))
        (map-set provider-stats { provider: tx-sender } {
          ...
          total-rewards: (get total-rewards stats), pool-count: (+ (get pool-count stats) u1),
...
(define-public (remove-liquidity (pool principal) (amount uint))
  (let ((position (unwrap! (map-get? liquidity-positions { pool: pool, provider: tx-sender }) (err ERR_NOT_PROVIDER))))
    ...
    (let ((stats (unwrap-panic (map-get? provider-stats { provider: tx-sender }))))
      (map-set provider-stats { provider: tx-sender } {
        total-liquidity: (- (get total-liquidity stats) amount),
        total-rewards: (get total-rewards stats), pool-count: (get pool-count stats), first-provision: (get first-provision stats)
      })
    )
...
(define-public (claim-rewards (pool principal))
  (let (
    ...
    (map-set liquidity-positions { pool: pool, provider: tx-sender } {
      liquidity-amount: (get liquidity-amount position), pool-shares: (get pool-shares position), last-deposit: (get last-deposit position), rewards-earned: (get rewards-earned position), rewards-claimed: (get rewards-earned position), fee-tier: (get fee-tier position)
    })

    (let ((stats (unwrap-panic (map-get? provider-stats { provider: tx-sender }))))
      (map-set provider-stats { provider: tx-sender } {
        total-liquidity: (get total-liquidity stats), total-rewards: (+ (get total-rewards stats) unclaimed),
        pool-count: (get pool-count stats), first-provision: (get first-provision stats)
      })
    )
