...
(define-public (deploy-from-template
    (template-id uint)
    (configuration (list 10 {
      key: (string-ascii 32),
      value: (string-ascii 64),
  ...
  (begin
    ...
    (let ((template-info (get-template template-id)))
      ...
      (let ((template (unwrap-optional template-info)))
        ...
        (asserts! (validate-configuration template configuration)
        ...
        (let ((instance-id (+ (var-get total-pools-created) u1)))
          ...
          (let ((pool-address (deploy-pool-contract template configuration)))
            ...
            (let ((stats (get-template-stats template-id)))
              (if (is-some stats)
                (map-set template-usage-stats { template-id: template-id } {
                  instances-created: (+ (get instances-created (get-optional stats)) u1),
                  total-liquidity: (get total-liquidity (get-optional stats)),
                  total-volume: (get total-volume (get-optional stats)),
                  average-fee: (get average-fee (get-optional stats)),
                  success-rate: (get success-rate (get-optional stats)),
                })
                (map-set template-usage-stats { template-id: template-id } {
...
(define-public (update-pool-configuration
    (pool principal)
    (configuration (list 10 {
      key: (string-ascii 32),
      value: (string-ascii 64),
...
(define-public (update-template-stats
    (template_id uint)
    (liquidity uint)
    (volume uint)
    (fee uint)
  ...
  (begin
    ...
    (let ((stats (get-template-stats template_id)))
      (if (is-some stats)
        (begin
          (let (
              (current-stats (unwrap-optional stats))
            ...
            (map-set template-usage-stats { template_id: template_id } {
              ...
              average-fee: (/ (+ (* (get current-stats average-fee) instances) fee)
                (+ instances u1)
              ),
...
(define-private (validate-configuration
    (template {
      name: (string-ascii 64),
      description: (string-ascii 256),
      pool-type: (string-ascii 32),
