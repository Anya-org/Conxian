  ...
  (begin
    ...
    (let ((vault-id (hash160 (sha256 0x00))))
      ...
      (map-set vaults { vault-id: vault-id } {
        ...
        tokens: tokens,
        created-at: (contract-call? .block-utils get-stacks-block-time),
      ...
      (print { event: "vault-created", vault-id: vault-id, owner: tx-sender, vault-type: vault-type, timestamp: (contract-call? .block-utils get-stacks-block-time) })
      (ok vault-id)
    )
  )
)
...
(define-public (deposit-to-vault (vault-id (buff 32)) (token-trait <sip-010-ft-trait>) (amount uint))
  (let (
    ...
    (let ((current-balance (default-to u0 (map-get? vault-balances { vault-id: vault-id, token: token }))))
      ...
      (print { event: "vault-deposited", vault-id: vault-id, token: token, amount: amount, timestamp: (contract-call? .block-utils get-stacks-block-time) })
      (ok true)
    )
  )
)
...
(define-public (withdraw-from-vault (vault-id (buff 32)) (token-trait <sip-010-ft-trait>) (amount uint))
  (let (
    ...
    ;; (contract-call? .block-utils restrict-assets-safe) returns true if successful
    (asserts!  (contract-call? .block-utils restrict-assets-safe) (err ERR_UNAUTHORIZED_ACCESS))
    ...
    (print { event: "vault-withdrawn", vault-id: vault-id, token: token, amount: amount, timestamp: (contract-call? .block-utils get-stacks-block-time) })
    (ok true)
  )
)
