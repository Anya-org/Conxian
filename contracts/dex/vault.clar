  ...
  (begin
    ...
    (let ((user_info (get-user-vaults tx-sender)))
      (if (is-some user_info)
          (asserts! (< (get-optional user_info).total-vaults MAX_VAULTS_PER_USER) ERR_VAULT_ALREADY_EXISTS)
          true
      )
    )
          ...
          (begin
            (let ((stats (unwrap-optional type_stats))
              ...
              (map-set vault-statistics { vault-type: vault_type } {
                ...
                average-balance: (get stats average-balance), // Would calculate actual average
                last-activity: block-height
              })
            )
          )
