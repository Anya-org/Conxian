...
(define-map memory-pools { pool-id: uint } { 
  pool-size: uint, allocated-slots: uint, free-slots: uint, last-cleanup: uint, pool-type: (string-ascii 16)
})

(define-map memory-allocations { pool-id: uint, slot-id: uint } {
  allocated-to: principal, allocation-size: uint, allocation-time: uint, last-access: uint, data: (buff 256)
})
  ...
  (begin
    ...
    (let ((pool-id (+ (var-get total-pools-allocated) u1)))
      (map-set memory-pools { pool-id: pool-id } {
        pool-size: pool-size, allocated-slots: u0, free-slots: pool-size, last-cleanup: burn-block-height, pool-type: pool-type
      })
    ...
    (let ((pool (unwrap! (map-get? memory-pools { pool-id: pool-id }) (err ERR_INVALID_ALLOCATION))))
      ...
      (let ((slot-id (+ (get allocated-slots pool) u1)))
        (map-set memory-allocations { pool-id: pool-id, slot-id: slot-id } {
          allocated-to: tx-sender, allocation-size: allocation-size, allocation-time: burn-block-height, last-access: burn-block-height, data: initial-data
        })
        
        (map-set memory-pools { pool-id: pool-id } {
          pool-size: (get pool-size pool), allocated-slots: slot-id, free-slots: (- (get free-slots pool) u1),
          last-cleanup: (get last-cleanup pool), pool-type: (get pool-type pool)
        })
  ...
  (begin
    (let ((alloc (unwrap! (map-get? memory-allocations { pool-id: pool-id, slot-id: slot-id }) (err ERR_INVALID_ALLOCATION))))
      ...
      (let ((pool (unwrap-panic (map-get? memory-pools { pool-id: pool-id }))))
        (map-set memory-pools { pool-id: pool-id } {
          pool-size: (get pool-size pool), allocated-slots: (- (get allocated-slots pool) u1),
