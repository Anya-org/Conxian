...
;; Conxian DEX: Memory pool management for efficient resource allocation
          ...
          (let ((user-index (get-user-allocations tx-sender)))
            (if (is-some user-index)
                (map-set allocation-index { principal: tx-sender } {
                  pool-id: pool-id,
                  slot-id: slot-id,
                  allocation-count: (+ (get-optional user-index).allocation-count u1)
                })
                (map-set allocation-index { principal: tx-sender } {
                  pool-id: pool-id,
                  slot-id: slot-id,
        ...
        (let ((pool-info (get-pool-info pool-id)))
          ...
          (let ((pool (unwrap-optional pool-info)))
            ...
            (let ((user-index (get-user-allocations tx-sender)))
              (if (is-some user-index)
                  (begin
                    (let ((new-count (- (get-optional user-index).allocation-count u1)))
                      (if (> new-count u0)
                          (map-set allocation-index { principal: tx-sender } {
                            pool-id: (get-optional user-index).pool-id,
                            slot-id: (get-optional user-index).slot-id,
