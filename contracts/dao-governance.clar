...
(define-public (create-proposal 
  ...
  (let (
    ...
    (let ((status (contract-call? .CXG-token get-migration-status)))
...
(define-public (check-and-reallocate-founder-tokens (founder principal) (epoch uint))
  (begin
    ...
    (let ((epoch-end-block (unwrap! (contract-call? .CXG-token get-epoch-end-block epoch) (err u701))))
    ...
    (let ((participation-rate (unwrap! (contract-call? .governance-metrics get-participation-rate founder epoch) (err u702))))
      ...
      (if (< participation-rate u60)
        (let ((founder-balance (unwrap! (contract-call? .CXG-token get-balance-of founder) (err u703)))
              (reallocation-amount (/ (* founder-balance u2) u100)))
          (try! (as-contract (contract-call? .CXG-token transfer-from founder .automated-bounty-system reallocation-amount)))
          (print {
...
;; u701: CXG-token-error
;; u702: governance-metrics-error
;; u703: CXG-token-balance-error
