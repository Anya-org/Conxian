  ...
  (let (
    ...
    (begin
      ...
      (let (
        ...
        (asserts! (>= current-dep amount) (err u1005))
        (asserts! (<= amount (get total-deposits reserve)) (err u1002))
...
(define-private (is-risk-manager)
  (is-eq contract-caller .risk-manager) ;; Or look up via conxian-protocol
)
