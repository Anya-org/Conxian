...
(define-data-var ops-engine-address principal tx-sender)
...
(define-public (set-ops-engine (new-address principal))
  (begin
    (asserts! (is-eq tx-sender (var-get contract-owner)) (err ERR_UNAUTHORIZED))
    (var-set ops-engine-address new-address)
    (ok true)
  )
)
...
(define-public (update-pid-rates)
  (begin
    (asserts! (or (is-eq contract-caller (var-get ops-engine-address)) (is-eq tx-sender (var-get contract-owner))) (err ERR_UNAUTHORIZED))
    (let (
...
(define-public (do-work (job-data (buff 2048)))
  (let (
    ...
    (begin
      ;; FIXME: Cannot call liquidate-position because we don't have the collateral-token trait available in do-work.
