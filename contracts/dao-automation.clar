...
(define-data-var CXG-token principal .CXG-token)
...
(define-public (create-emergency-vote (action (string-ascii 50)) (parameters (list 5 uint)))
  (begin
    (let (
      ...
      (CXG-balance (unwrap! (contract-call? .CXG-token get-balance-of tx-sender) (err u300)))
      (total-supply (unwrap! (contract-call? .CXG-token get-total-supply) (err u301)))
      (voting-power-bps (/ (* CXG-balance u10000) total-supply))
    )
...
(define-public (vote-emergency (vote-id uint) (support bool))
  (let (
    ...
    (voter-balance (unwrap! (contract-call? .CXG-token get-balance-of tx-sender) (err u304)))
  )
...
(define-public (execute-emergency-vote (vote-id uint))
  (let (
    ...
    (total-supply (unwrap! (contract-call? .CXG-token get-total-supply) (err u308)))
...
(define-public (generate-epoch-report)
  (begin
    ...
    (let (
      ...
      (CXG-holders (get-CXG-holder-count))
...
(define-private (get-CXG-holder-count)
  ;; Query token contract for holder count approximation using direct contract call
  (let (
    (total-supply (unwrap-panic (contract-call? .CXG-token get-total-supply)))
  )
