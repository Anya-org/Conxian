      ...
      (map-set gas-optimization-stats
        ...
        (merge current-stats 
               {optimization-applied: true,
                ...
                avg-gas-per-call: (if (> (get avg-gas-per-call current-stats) gas-savings)
                                   (- (get avg-gas-per-call current-stats) gas-savings)
                                   (get avg-gas-per-call current-stats))}))
...
(define-private (calculate-gas-savings (stats (tuple (total-calls uint) (total-gas-used uint) (avg-gas-per-call uint) (optimization-applied bool) (last-optimized uint))) (optimization-level uint))
  (let ((base-savings (/ (get avg-gas-per-call stats) u10))) ;; 10% base savings
...
(define-public (update-system-load
  ...
  (total-gas uint)
  (avg-execution-time uint))
  (let ((congestion-level (calculate-congestion-level transaction-count total-gas avg-execution-time)))
...
(define-read-only (estimate-gas-savings (contract principal) (function-name (string-ascii 30)))
  (match (get-gas-optimization-stats contract function-name)
    stats (let ((avg-gas (get avg-gas-per-call stats))
                (optimization-level (var-get gas-optimization-level)))
