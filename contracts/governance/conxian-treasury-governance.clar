;; conxian-treasury-governance.clar
;;
;; This contract manages the governance process for the Conxian Protocol's
;; treasury. Participation requires holding both a Governance Seat NFT and
;; the CXTR token.
...
(define-data-var cxtr-token-contract principal .cxtr-token)
...
(define-private (is-authorized-voter (voter principal))
  (let
    (
      (cxtr-balance (unwrap! (contract-call? (var-get cxtr-token-contract) get-balance voter) (err u0)))
