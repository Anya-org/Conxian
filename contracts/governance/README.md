...
- **Active governance path**: `proposal-engine.clar` routes proposals, `proposal-registry.clar` records them, `proposal-executor.clar` handles execution, and `timelock.clar` enforces delayed execution.
...
Active proposals should follow `proposal-engine` → `proposal-registry` → `proposal-executor` → `timelock`. The community-voting and upgrade-controller stubs have no supported integration until their architecture and APIs are designed.
...
- **Proposal path**: The active sequence from proposal submission through registry, execution, and timelock enforcement.
...
## Status (Reference)
- Active path: `proposal-engine`, `proposal-registry`, `proposal-executor`, and `timelock`.
