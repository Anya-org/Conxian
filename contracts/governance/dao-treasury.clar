...
(define-public (deposit (amount uint) (token .sip-standards.sip-010-ft-trait))
    (contract-call? token transfer amount tx-sender (as-contract tx-sender) none)
)

(define-public (withdraw (amount uint) (token .sip-standards.sip-010-ft-trait))
    (begin
        (asserts! (is-owner) ERR_UNAUTHORIZED)
...
(define-read-only (get-balance (token .sip-standards.sip-010-ft-trait))
