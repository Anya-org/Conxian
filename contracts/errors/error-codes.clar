...
(define-read-only (get-error-name (code uint))
  (match code
    u1000 (ok "ERR_UNAUTHORIZED")
    u1001 (ok "ERR_PAUSED")
    u1002 (ok "ERR_INSUFFICIENT_LIQUIDITY")
    u1003 (ok "ERR_INVALID_AMOUNT")
    u1004 (ok "ERR_INSUFFICIENT_COLLATERAL")
    u1005 (ok "ERR_INVALID_PROOF")
    u2001 (ok "ERR_INVALID_TICK")
    u3001 (ok "ERR_PROPOSAL_NOT_FOUND")
    u6001 (ok "ERR_BITVM2_VERIFICATION_FAILED")
    u6007 (ok "ERR_NOT_VERIFIER")
    u7001 (ok "ERR_TREASURY_UNAUTHORIZED")
    u8001 (ok "ERR_NON_COMPLIANT")
    u9001 (ok "ERR_ORACLE_STALE")
    (ok "ERR_UNKNOWN")
  )
)
