...
(define-public (withdraw
  ...
  (begin
    ...
    (as-contract (try! (contract-call? token transfer amount tx-sender none)))
...
(define-public (get-reward (token <sip-010-ft-trait>))
  (let ((reward (earned tx-sender)))
    ...
    (if (> reward u0)
      (as-contract (try! (contract-call? token transfer reward tx-sender none)))
      true
    )
...
(define-public (notify-reward-amount (amount uint) (token <sip-010-ft-trait>))
  (let (
    ...
    (begin
      ...
      (print { event: "notify-reward", amount: amount, rate: (var-get reward-rate), timestamp: burn-block-height })
      (ok true)
    )
  )
)
