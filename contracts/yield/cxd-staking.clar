...
(define-private (update-reward (account principal))
    (let (
        (new-per-token (get-reward-per-token))
    )
...
(define-public (stake (amount uint) (token <sip-010-trait>))
    (begin
        ;; Fail if paused
...
(define-public (get-reward (token <sip-010-trait>))
    (let (
        ...
        (asserts! (check-compliance tx-sender) ERR_NON_COMPLIANT)
        (asserts! (is-eq (contract-of token) (var-get rewards-token)) ERR_UNAUTHORIZED)
        ...
        (print { event: "get-reward", user: tx-sender, amount: reward })
        (ok reward)
    )
)

(define-public (exit (staking-token-trait <sip-010-trait>) (rewards-token-trait <sip-010-trait>))
    (begin
