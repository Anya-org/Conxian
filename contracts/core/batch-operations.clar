...
(define-map batch-results
  { batch-id: uint }
  {
    success-count: uint, failure-count: uint, gas-used: uint, timestamp: uint
  }
)
...
(define-public (process-batch
    (operations (list
      50
      {
        type: uint, target: principal, function: (string-ascii 32), params: (list 5 principal), gas-limit: uint
      }
    ))
  ...
  (begin
    ...
    (let ((actual-batch-id (default-to (var-get current-batch-id) batch-id)))
      ...
      (let ((result (execute-batch-with-gas-tracking operations actual-batch-id)))
        ...
        (print {
          event: "batch-completed", batch-id: actual-batch-id, operations-count: (len operations), result: result, timestamp: burn-block-height
        })
...
(define-private (execute-batch-with-gas-tracking
  ...
  (let (
    ...
    (map-set batch-results { batch-id: batch-id } {
      success-count: (match results val val err u0), failure-count: (match results val u0 err u1), gas-used: u0, timestamp: burn-block-height
    })
...
(define-read-only (get-batch-statistics)
  (ok {
    total-batches: (var-get total-batches-processed), current-batch-id: (var-get current-batch-id), batch-enabled: (var-get batch-enabled), max-batch-size: MAX_BATCH_SIZE
  })
)
