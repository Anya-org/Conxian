...
;; Migrated to block-height for tenure-precision vesting.
  ...
  (let ((schedule (map-get? vesting-schedules tx-sender)))
    ...
    (let ((vested-amount (calculate-vested-amount (unwrap! schedule (err ERR_NO_VESTING_SCHEDULE)))))
      ...
      (let ((claim-amount (- vested-amount (get claimed-amount (unwrap! schedule (err ERR_NO_VESTING_SCHEDULE))))))
        ...
        (print { event: "vesting-claimed", beneficiary: tx-sender, amount: claim-amount, timestamp: block-height })
        (ok claim-amount)
      )
    )
  )
...
(define-private (calculate-vested-amount (schedule {total-amount: uint, start-time: uint, end-time: uint, claimed-amount: uint}))
  (if (< block-height (var-get vesting-start))
    u0
    (if (>= block-height (+ (var-get vesting-start) (- (get end-time schedule) (get start-time schedule))))
      (get total-amount schedule)
      (/ (* (get total-amount schedule) (- block-height (var-get vesting-start))) (- (get end-time schedule) (get start-time schedule)))
    )
  )
)
