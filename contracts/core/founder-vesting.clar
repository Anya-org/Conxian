...
;; Migrated to (contract-call? .block-utils get-stacks-block-time) for second-precision vesting.
...
(define-public (initialize (owner principal))
  (begin
    ...
    (var-set vesting-start (contract-call? .block-utils get-stacks-block-time))
    (ok true)
  )
)
  ...
  (let ((schedule (map-get? vesting-schedules tx-sender)))
    ...
    (let ((vested-amount (calculate-vested-amount (unwrap! schedule (err ERR_NO_VESTING_SCHEDULE)))))
      ...
      (let ((claim-amount (- vested-amount (get claimed-amount (unwrap! schedule (err ERR_NO_VESTING_SCHEDULE))))))
        ...
        (print { event: "vesting-claimed", beneficiary: tx-sender, amount: claim-amount, timestamp: (contract-call? .block-utils get-stacks-block-time) })
        (ok claim-amount)
      )
    )
  )
...
(define-private (calculate-vested-amount (schedule {total-amount: uint, start-time: uint, end-time: uint, claimed-amount: uint}))
  (if (< (contract-call? .block-utils get-stacks-block-time) (var-get vesting-start))
    u0
    (if (>= (contract-call? .block-utils get-stacks-block-time) (+ (var-get vesting-start) (- (get end-time schedule) (get start-time schedule))))
      (get total-amount schedule)
      (/ (* (get total-amount schedule) (- (contract-call? .block-utils get-stacks-block-time) (var-get vesting-start))) (- (get end-time schedule) (get start-time schedule)))
    )
  )
)
