...
(define-private (is-admin-or-dao)
  (or 
    (is-eq tx-sender (var-get dao-governance))
    (is-eq tx-sender (var-get treasury))
    (is-eq contract-caller (var-get dao-governance))
  )
)
...
;; u410: proof-too-short
;; u411: proof-not-found
;; u412: dispute-period-active
;; u413: already-verified
;; u414: dispute-period-expired
;; u415: dispute-not-found
...
(define-map dispute-records
  { dispute-id: uint }
  {
    ...
    resolved: bool,
    resolution: (optional bool) ;; true=valid, false=invalid, none=pending
  })
...
(define-public (submit-milestone-proof (bounty-id uint) (milestone uint) (proof-hash (buff 32)))
  (let (
    ...
    (milestone-data (unwrap! (map-get? milestones { bounty-id: bounty-id, milestone-id: milestone }) (err u112)))
  )
    ;; Verify bounty is assigned to sender
    (asserts! (is-eq (get assignee bounty) (some tx-sender)) (err u113))
    ;; Verify milestone exists and isn't completed
    (asserts! (not (is-eq (get status milestone-data) MILESTONE_STATUS_APPROVED)) (err u108))
    ;; Verify proof hash length
    (asserts! (>= (len proof-hash) MIN_PROOF_LENGTH) (err u410))
    
    ;; Store proof with dispute period
    (map-set submission-proofs { bounty-id: bounty-id, milestone: milestone }
      {
        proof-hash: proof-hash,
        timestamp: block-height,
        verified: false,
        dispute-period-end: (+ block-height u144) ;; 24 hour dispute period
      })
    
    (ok true)))

;; Verify milestone after dispute period
(define-public (verify-milestone (bounty-id uint) (milestone uint))
  (let (
    (proof (unwrap! (map-get? submission-proofs { bounty-id: bounty-id, milestone: milestone }) (err u411)))
    (milestone-data (unwrap! (map-get? milestones { bounty-id: bounty-id, milestone-id: milestone }) (err u112)))
  )
    ;; Check dispute period has ended
    (asserts! (>= block-height (get dispute-period-end proof)) (err u412))
    ;; Verify not already verified
    (asserts! (not (get verified proof)) (err u413))
    
    ;; Mark milestone as completed and proof as verified
    (map-set milestones { bounty-id: bounty-id, milestone-id: milestone }
      (merge milestone-data { status: MILESTONE_STATUS_APPROVED }))
    
    (map-set submission-proofs { bounty-id: bounty-id, milestone: milestone }
      (merge proof { verified: true }))
    
    ;; Pay milestone reward
    (try! (pay-milestone-reward bounty-id milestone))
    (ok true)))

;; Pay milestone reward (helper function)
(define-private (pay-milestone-reward (bounty-id uint) (milestone uint))
  (let (
    (bounty (unwrap! (map-get? bounties { id: bounty-id }) (err u107)))
    (milestone-data (unwrap! (map-get? milestones { bounty-id: bounty-id, milestone-id: milestone }) (err u112)))
    (milestone-reward (/ (* (get reward-amount bounty) (get reward-percentage milestone-data)) u100))
    (assignee (unwrap! (get assignee bounty) (err u113)))
  )
    ;; Transfer milestone payment - simplified for now
    ;; (unwrap! (as-contract (contract-call? .treasury pay-milestone 
    ;;   assignee milestone-reward)) (err u200))
    (ok milestone-reward)))

;; Challenge a submitted proof
(define-public (challenge-proof (bounty-id uint) (milestone uint) (evidence (string-utf8 500)))
  (let (
    (proof (unwrap! (map-get? submission-proofs { bounty-id: bounty-id, milestone: milestone }) (err u411)))
    (dispute-id (+ (var-get dispute-counter) u1))
  )
    ;; Check dispute period is still active
    (asserts! (< block-height (get dispute-period-end proof)) (err u414))
    ;; Verify proof not already verified
    (asserts! (not (get verified proof)) (err u413))
    
    ;; Create dispute record
    (map-set dispute-records { dispute-id: dispute-id }
      {
        bounty-id: bounty-id,
        milestone: milestone,
...
(define-public (resolve-dispute (dispute-id uint) (valid bool))
  (let (
    (dispute (unwrap! (map-get? dispute-records { dispute-id: dispute-id }) (err u415)))
  )
    ;; Only admin can resolve
    (asserts! (is-admin-or-dao) (err u100))
    ;; Verify not already resolved
    (asserts! (not (get resolved dispute)) (err u416))
    
    ;; Mark dispute as resolved
    (map-set dispute-records { dispute-id: dispute-id }
      (merge dispute { resolved: true, resolution: (some valid) }))
    
    ;; If proof is invalid, extend dispute period
    (if (not valid)
      (let (
        (proof (unwrap! (map-get? submission-proofs 
          { bounty-id: (get bounty-id dispute), milestone: (get milestone dispute) }) (err u411)))
      )
        (map-set submission-proofs 
          { bounty-id: (get bounty-id dispute), milestone: (get milestone dispute) }
          (merge proof { dispute-period-end: (+ block-height u144) })))
      true)
