...
(define-public (request-unstake (position-id uint) (amount uint) (owner principal))
  (let ((position (unwrap! (map-get? positions position-id) ERR_INVALID_STATE)))
    (begin
      (asserts! (not (var-get fail-request)) ERR_FORCED_FAILURE)
      (asserts! (is-eq (get owner position) owner) ERR_OWNER_MISMATCH)
      (asserts! (is-eq (get amount position) amount) ERR_AMOUNT_MISMATCH)
      (asserts! (is-eq (get status position) u1) ERR_INVALID_STATE)
      (map-set positions position-id (merge position { status: u2 }))
      (ok true)
    )
  )
)
...
(define-public (finalize-unstake (position-id uint) (amount uint) (owner principal))
  (let ((position (unwrap! (map-get? positions position-id) ERR_INVALID_STATE)))
    (begin
      ...
      (asserts! (is-eq (get owner position) owner) ERR_OWNER_MISMATCH)
      (asserts! (is-eq (get amount position) amount) ERR_AMOUNT_MISMATCH)
      (asserts! (is-eq (get status position) u2) ERR_INVALID_STATE)
      (map-set positions position-id (merge position { status: u3 }))
      (ok amount)
    )
  )
)
