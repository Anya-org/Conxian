...
(define-map outgoing-messages
    uint
    {
        target-chain: uint, recipient: (buff 32), payload: (buff 1024), sender: principal, sent-at: uint
    }
)
  ...
  (let ((seq (+ (var-get sequence) u1)))
    (begin
      ...
      (map-set outgoing-messages seq {
          target-chain: target-chain, recipient: recipient, payload: payload, sender: tx-sender, sent-at: burn-block-height
      })
