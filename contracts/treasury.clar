...
(define-data-var total-CXG-bought uint u0)
...
(define-trait dex-trait
  (
    (swap-stx-for-CXG (uint) (response uint uint))
  )
)
  ...
  (begin
    ...
    (let (
      ...
      (let ((buyback-amount (if (< max-buyback (/ treasury-balance u20)) max-buyback (/ treasury-balance u20))))
          ...
          (let ((estimated-CXG-bought buyback-amount)) ;; Use buyback amount as estimated CXG bought
            (var-set total-CXG-bought (+ (var-get total-CXG-bought) estimated-CXG-bought))
            (print { event: "auto-buyback-executed", stx-spent: buyback-amount, estimated-CXG-bought: estimated-CXG-bought, treasury-balance: (var-get stx-reserve), block: block-height })
            (ok estimated-CXG-bought)
          )
        )
      )
    )
...
(define-read-only (get-buyback-status)
  {
    ...
    total-CXG-bought: (var-get total-CXG-bought),
    buyback-ready: (and 
...
(define-public (execute-spending (proposal-id uint))
  (let (
    ...
    (if (is-eq (get asset-type proposal) u0)
      ...
      ;; CXG token transfer  
      (try! (as-contract (contract-call? .CXG-token transfer (get recipient proposal) (get amount proposal)))))
