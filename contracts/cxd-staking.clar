...
(define-public (complete-unstake)
  (match (get-pending-unstake tx-sender)
    unstake-info
    (begin
      ...
      (let ((cxd-amount (get cxd-amount unstake-info))
            (cxd-contract (var-get cxd-token-contract)))
        
        ;; Transfer CXD back to user
        (try! (as-contract (contract-call? cxd-contract transfer cxd-amount (as-contract tx-sender) tx-sender none)))
