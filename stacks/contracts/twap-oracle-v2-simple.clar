...
(define-private (compute-average-last-n (pair {token-a: principal, token-b: principal}) (n uint))
  ;; Returns {avg, count} over up to last n (capped at MAX_SAMPLE) observations.
...
(define-private (check-for-manipulation (pair {token-a: principal, token-b: principal}) (current-price uint) (current-volume uint))
  (let ((avg-info (compute-average-last-n pair u5)))
    (if (>= (get count avg-info) u2)
      (let ((avg-price (get avg avg-info))
            (deviation (if (> current-price (get avg avg-info))
                        (/ (* (- current-price (get avg avg-info)) u10000) (max-u u1 (get avg avg-info)))
                        (/ (* (- (get avg avg-info) current-price) u10000) (max-u u1 (get avg avg-info))))))
      ...
      (let (
        ...
        (let (
          ...
          (if (is-eq cnt5 u0)
            ...
            (let ((avg (/ sum5 cnt5)))
              (begin
                (map-set twap-cache {pair: pair, period: period} {twap-price: avg, calculated-at: block-height, valid-until: (+ block-height (min period u10)), observations-used: cnt5})
                (ok avg)))))))))
