...
const EXPECTED_ARTIFACTS = {
  ...
  "emily-public-openapi": {
    ...
    sourcePath: "emily/openapi-gen/generated-specs/public-emily-openapi-spec.json",
    sha256: "9d8f1236015c38b04f8b1a75915ac66c4fcdd78c7aa922c7ab6700344aff2147",
  },
} as const;
const REQUIRED_ARTIFACTS = ["withdrawal-contract", "registry-contract", "emily-public-openapi"];
...
function addReason(reasons: EvidenceReason[], code: string, message: string): void {
...
export function validateRecipientTuple(tuple: RecipientTuple): boolean {
  if (!Number.isInteger(tuple.version) || tuple.version < 0 || tuple.version > 6) return false;
  if (!/^[0-9a-fA-F]+$/.test(tuple.hashbytes) || tuple.hashbytes.length % 2 !== 0) return false;
...
export function expectedWithdrawalLock(amount: bigint, maxFee: bigint): bigint {
  if (amount < 0n || maxFee < 0n) throw new Error("amount and maxFee must be non-negative");
...
export function evaluateUnsignedWithdrawalGate(
  ...
  if (!validateRecipientTuple(input.recipient)) reasons.push("INVALID_RECIPIENT_TUPLE");
  if (input.token.principal === "unresolved" || !input.token.chainVerified) {
    reasons.push("TOKEN_PRINCIPAL_NOT_CHAIN_VERIFIED");
  }
  if (input.withdrawal.principal === "unresolved" || !input.withdrawal.chainVerified) {
...
export function verifyEvidencePack(options: VerifyOptions = {}): EvidenceDecision {
  const rootDir = resolve(options.rootDir ?? process.cwd());
  const manifestPath = resolve(rootDir, options.manifestPath ?? DEFAULT_MANIFEST);
  ...
  const repository = typeof upstream.repository === "string" ? upstream.repository : "";
  if (!/^v\d+\.\d+\.\d+$/.test(release) || ["main", "master", "latest"].includes(release)) {
    addReason(offlineReasons, "FLOATING_UPSTREAM_RELEASE", "Pin a semantic upstream release tag.");
  }
  if (!/^[0-9a-f]{40}$/.test(commit)) {
    addReason(offlineReasons, "MISSING_IMMUTABLE_COMMIT", "Pin a full 40-character upstream commit SHA.");
  }
  if (
  ...
  for (const value of artifacts) {
    ...
    const path = typeof artifact.path === "string" ? resolve(rootDir, artifact.path) : "";
    const sourceUrl = typeof artifact.sourceUrl === "string" ? artifact.sourceUrl : "";
    const expectedHash = typeof artifact.sha256 === "string" ? artifact.sha256 : "";
    const expectedArtifact = EXPECTED_ARTIFACTS[kind as keyof typeof EXPECTED_ARTIFACTS];
    ...
    if (!path || !sourceUrl.includes(commit) || !/^https:\/\//.test(sourceUrl)) {
      addReason(
        offlineReasons,
    ...
    try {
      ...
      addReason(offlineReasons, "ARTIFACT_MISSING", `${kind}: ${String(error)}`);
    }
  }
  for (const kind of REQUIRED_ARTIFACTS) {
    if (!artifactByKind.has(kind)) {
      addReason(offlineReasons, "REQUIRED_ARTIFACT_MISSING", `Missing required ${kind} evidence.`);
    }
  }
  ...
  if (withdrawalArtifact && typeof withdrawalArtifact.path === "string") {
    try {
      const source = readFileSync(resolve(rootDir, withdrawalArtifact.path), "utf8");
      ...
      if (!requiredAbiFragments.every((fragment) => source.includes(fragment))) {
        addReason(offlineReasons, "WITHDRAWAL_ABI_EVIDENCE_INVALID", "Pinned withdrawal ABI evidence is incomplete.");
      }
    } catch (error) {
      addReason(offlineReasons, "WITHDRAWAL_ABI_EVIDENCE_UNREADABLE", String(error));
    }
  }
  ...
  if (registryArtifact && typeof registryArtifact.path === "string") {
    try {
      const source = readFileSync(resolve(rootDir, registryArtifact.path), "utf8");
      if (!source.includes("(define-map withdrawal-status uint bool)") || !source.includes("status: (map-get? withdrawal-status id)")) {
        addReason(offlineReasons, "REGISTRY_STATE_EVIDENCE_INVALID", "Pinned registry status evidence is incomplete.");
      }
    } catch (error) {
      addReason(offlineReasons, "REGISTRY_STATE_EVIDENCE_UNREADABLE", String(error));
    }
  }
  ...
  if (emilyArtifact && typeof emilyArtifact.path === "string") {
    try {
      ...
      const schemas = asObject(components.schemas, "openapi.components.schemas");
      const withdrawalStatus = asObject(schemas.WithdrawalStatus, "openapi.WithdrawalStatus");
      if (!sameStrings(stringArray(withdrawalStatus.enum), EMILY_STATES)) {
        addReason(offlineReasons, "EMILY_STATUS_SCHEMA_INVALID", "Emily withdrawal states do not match the pinned schema.");
      }
    } catch (error) {
  ...
  if (vectors.length === 0) {
    addReason(offlineReasons, "RECIPIENT_VECTORS_MISSING", "Recipient tuple vectors are required.");
  }
  for (const value of vectors) {
    ...
    if (actual !== vector.valid) {
      addReason(offlineReasons, "RECIPIENT_VECTOR_MISMATCH", `${name} expected valid=${String(vector.valid)}.`);
    }
  }
  ...
  if (matrix.upstreamRelease !== release || matrix.upstreamCommit !== commit) {
    addReason(integrationReasons, "MATRIX_PIN_MISMATCH", "Matrix pin must match the snapshot manifest.");
  }
  if (matrix.targetNetwork === "unresolved" || typeof matrix.targetNetwork !== "string") {
    addReason(integrationReasons, "TARGET_NETWORK_UNRESOLVED", "Select and verify the official target network.");
  }
  const requiredEvidence = Array.isArray(matrix.requiredEvidence) ? matrix.requiredEvidence : [];
  const cells = new Map<string, JsonObject>();
  for (const value of requiredEvidence) {
    const cell = objectOrReason(value, "matrix cell", integrationReasons, "MATRIX_CELL_INVALID");
    if (typeof cell.id === "string") cells.set(cell.id, cell);
  }
  for (const id of REQUIRED_MATRIX_CELLS) {
    const cell = cells.get(id);
    if (!cell) {
      addReason(integrationReasons, "MATRIX_CELL_MISSING", `Add required matrix cell: ${id}.`);
      continue;
    }
    const unresolved = Object.values(cell).includes("unresolved");
    const verified = cell.kind === "service" ? cell.liveVerified === true : cell.chainVerified === true;
    if (unresolved || !verified) {
      addReason(integrationReasons, "MATRIX_CELL_UNRESOLVED", `${id} requires authoritative live verification.`);
    }
  }
  ...
  if (!sameStrings(stringArray(stateModels.registryWithdrawal), REGISTRY_STATES)) {
    addReason(integrationReasons, "REGISTRY_STATES_INVALID", "Registry states must be pending|accepted|rejected.");
  }
  if (!sameStrings(stringArray(stateModels.emilyWithdrawal), EMILY_STATES)) {
    addReason(integrationReasons, "EMILY_STATES_INVALID", "Emily states must be pending|accepted|confirmed|failed.");
  }
  ...
  for (const [field, value] of Object.entries(bitcoinOutputEvidence)) {
    if (value === "unresolved") {
      addReason(settlementReasons, "BITCOIN_OUTPUT_EVIDENCE_UNRESOLVED", `${field} remains unresolved.`);
    }
  }
  addReason(
    settlementReasons,
  ...
  const reasons = [...offlineReasons, ...integrationReasons, ...settlementReasons].sort((a, b) =>
  ...
  return {
    ...
    officialNetworkIntegration:
      offlineReasons.length === 0 && integrationReasons.length === 0 ? "GO" : "NO-GO",
    bitcoinSettlementClaim: "NO-GO",
    reasons,
  };
}
