...
## 13. Recent Alignment (v0.6.1 Patch)

### 13.1. Compliance Module Refactor
- Consolidated KYC/AML logic into a unified `compliance-hooks` and `kyc-registry` system.
- Standardized error codes (u7000 series) for compliance violations.
- Implemented static contract calls for critical compliance checks to ensure deterministic performance.

### 13.2. Autonomous Agent Stability
- Resolved circular dependencies between `ops-engine`, `agent-risk`, and `agent-treasury`.
- Aligned `liquidate` signatures across `risk-manager-trait`, `agent-risk`, and `risk-manager`.
- Synchronized `agent-treasury` distribution logic with the Dual-Council governance model.

### 13.3. Test Suite Synchronization
- Automated stubbing for peripheral modules (`enterprise-facade`, `mock-usda-token`) to maintain 100% pass rate on core protocol journeys.
- Updated `Clarinet.toml` with precise dependency graphs for all 40+ contracts.
