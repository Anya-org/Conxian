...
function updateJson(root: string, relativePath: string, mutate: (value: any) => void): void {
...
describe("Phase 2 sBTC evidence harness", () => {
  it("grants GO only to the pinned offline snapshot and remains NO-GO elsewhere", () => {
    ...
    expect(decision.reasons.filter((reason) => reason.code === "MATRIX_CELL_UNRESOLVED")).toHaveLength(7);
    expect(decision.reasons.some((reason) => reason.code === "OFFLINE_HARNESS_CANNOT_PROVE_SETTLEMENT")).toBe(true);
  });

  it("fails closed on floating or missing upstream pins", () => {
    const root = isolatedEvidence();
    updateJson(root, "evidence/sbtc/phase2/v1.3.3/manifest.json", (manifest) => {
    ...
    expect(decision.reasons.map((reason) => reason.code)).toEqual(
      expect.arrayContaining(["FLOATING_UPSTREAM_RELEASE", "MISSING_IMMUTABLE_COMMIT"]),
    );
  });
  ...
  it("rejects an alternate valid-looking release and full commit", () => {
    ...
    updateJson(root, "evidence/sbtc/phase2/v1.3.3/target-network-matrix.json", (matrix) => {
    ...
    expect(decision.offlineSnapshot).toBe("NO-GO");
    expect(decision.reasons.map((reason) => reason.code)).toEqual(
      expect.arrayContaining(["UNEXPECTED_UPSTREAM_PIN", "ARTIFACT_PROVENANCE_MISMATCH"]),
    );
  });

  it("fails closed on snapshot hash mismatches", () => {
    const root = isolatedEvidence();
    const withdrawal = join(root, "evidence/sbtc/phase2/v1.3.3/sources/sbtc-withdrawal.clar");
    writeFileSync(withdrawal, `${readFileSync(withdrawal, "utf8")}\n;; tampered\n`, "utf8");

    const decision = verifyEvidencePack({ rootDir: root });
    expect(decision.offlineSnapshot).toBe("NO-GO");
    expect(decision.reasons.some((reason) => reason.code === "ARTIFACT_HASH_MISMATCH")).toBe(true);
  });
  ...
  it("requires withdrawal ABI, registry, and public Emily evidence", () => {
    ...
    updateJson(root, "evidence/sbtc/phase2/v1.3.3/manifest.json", (manifest) => {
      manifest.artifacts = manifest.artifacts.filter(
        (artifact: { kind: string }) => artifact.kind !== "withdrawal-contract" && artifact.kind !== "emily-public-openapi",
      );
    });

    const decision = verifyEvidencePack({ rootDir: root });
    expect(decision.offlineSnapshot).toBe("NO-GO");
    expect(decision.reasons.filter((reason) => reason.code === "REQUIRED_ARTIFACT_MISSING")).toHaveLength(2);
  });
  ...
  it("returns structured NO-GO when a required source file is unreadable", () => {
    ...
    updateJson(root, "evidence/sbtc/phase2/v1.3.3/manifest.json", (manifest) => {
      const withdrawal = manifest.artifacts.find(
        (artifact: { kind: string }) => artifact.kind === "withdrawal-contract",
      );
      withdrawal.path = "evidence/sbtc/phase2/v1.3.3/sources/missing-withdrawal.clar";
    });
  ...
  it("requires complete recipient-vector boundary coverage", () => {
    const root = isolatedEvidence();
    updateJson(root, "evidence/sbtc/phase2/v1.3.3/recipient-vectors.json", (document) => {
      document.vectors = document.vectors.filter((vector: { version: number; valid: boolean }) => {
    ...
    const decision = verifyEvidencePack({ rootDir: root });
    expect(decision.offlineSnapshot).toBe("NO-GO");
    expect(decision.reasons.some((reason) => reason.code === "RECIPIENT_VECTOR_COVERAGE_MISSING")).toBe(true);
  });

  it("keeps registry and Emily states separate", () => {
    const root = isolatedEvidence();
    updateJson(root, "evidence/sbtc/phase2/v1.3.3/target-network-matrix.json", (matrix) => {
      matrix.stateModels.registryWithdrawal = matrix.stateModels.emilyWithdrawal;
    });

    const decision = verifyEvidencePack({ rootDir: root });
    expect(decision.officialNetworkIntegration).toBe("NO-GO");
    expect(decision.reasons.some((reason) => reason.code === "REGISTRY_STATES_INVALID")).toBe(true);
  });
  ...
  it("rejects any sole-proof settlement claim, including Emily confirmed", () => {
    ...
    const decision = verifyEvidencePack({ rootDir: root });
    expect(decision.bitcoinSettlementClaim).toBe("NO-GO");
    expect(decision.reasons.some((reason) => reason.code === "PROHIBITED_SOLE_SETTLEMENT_PROOF")).toBe(true);
  });
});
...
describe("withdrawal recipient tuple vectors", () => {
  for (const version of [0, 1, 2, 3, 4]) {
    it(`accepts version 0x${version.toString(16).padStart(2, "0")} only with 20 bytes`, () => {
      expect(validateRecipientTuple({ version, hashbytes: "ab".repeat(20) })).toBe(true);
      expect(validateRecipientTuple({ version, hashbytes: "ab".repeat(32) })).toBe(false);
    });
  }

  for (const version of [5, 6]) {
    it(`accepts version 0x${version.toString(16).padStart(2, "0")} only with 32 bytes`, () => {
      expect(validateRecipientTuple({ version, hashbytes: "cd".repeat(32) })).toBe(true);
      expect(validateRecipientTuple({ version, hashbytes: "cd".repeat(20) })).toBe(false);
    });
  }

  it("rejects unsupported versions and malformed tuple bytes without parsing address strings", () => {
    expect(validateRecipientTuple({ version: 7, hashbytes: "ef".repeat(32) })).toBe(false);
    expect(validateRecipientTuple({ version: 4, hashbytes: "not-an-address" })).toBe(false);
  });
});
...
describe("unsigned withdrawal construction gate", () => {
  ...
  it("does not fabricate transaction bytes even after the evidence gate shape is satisfied", () => {
    const result = evaluateUnsignedWithdrawalGate({
      ...
      withdrawal: { principal: "chain-verified-by-caller", chainVerified: true },
    });

    expect(result.allowed).toBe(false);
    expect(result.reasons).toEqual(["UNSIGNED_TRANSACTION_BUILDER_INTENTIONALLY_NOT_IMPLEMENTED"]);
