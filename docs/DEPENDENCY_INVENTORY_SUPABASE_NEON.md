...
| `Sovereign-Ops-Orchestrator` (Ops Engine) | **Policy doc only**: `LINEAR_WIRING.md` defines public-safe Linear-first ExCo intake/routing (no BOS webhook state wiring details) | Operational intake/routing governance for ExCo work | **Governance** | Linear issue intake/triage workflows | **Spec/Doc**: `Sovereign-Ops-Orchestrator/LINEAR_WIRING.md` |
