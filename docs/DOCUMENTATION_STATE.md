...
- Added the injected PoX/stacking adapter boundary, delegated native operator,
  dual-stacking accounting orchestrator, deterministic mocks, and focused
  Vitest coverage under `tests/staking/`.
- Updated `contracts/staking/README.md` and added
  `docs/DUAL_STACKING_ARCHITECTURE.md` with the generic SIP-010, burn-height,
  BTC-attestation, reserve, risk, pause, and non-deployment boundaries.
- Clarinet manifests are updated. Release YAML regeneration remains blocked by
  the unavailable pinned Clarinet CLI; the SDK-generated working-tree plan is
  not committed because it changes unrelated sender/version data.
