# Proposal: Sovereign Data Migration & Sovereign Egress Architecture (CON-343)
...
Separately, treasury and gateway work has begun to formalize sovereign accounting outputs (settlement logs, ISO 20022 outputs). This proposal frames that work as a standardized **read-only subledger egress** surface, not "custom ERP plugin" work.
...
3. **Sovereign egress standard**: position `conxian-nexus` as a read-only, cryptographically verifiable **Glass Node** for external subledgers, with `conxian-gateway` as the sovereign API surface.
...
- Add a focused OpenSpec package that:
  ...
  - defines "sovereign egress" as standardized, read-only subledger export.
...
- Existing SAB migration issues become more concrete about:
  ...
  - and what "sovereign egress" means in terms of datasets, proofs, and API surfaces.
